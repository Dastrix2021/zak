...
public class Main {
    ...
    public static void main(String[] args) throws IOException, NoSuchFieldException {
        Parser p = new Parser();
        Excel ex = new Excel();
        List<String> idd = List.copyOf(ex.excel()); //КОПИЯ ЛИСТА ЕКСЕЛЬ
        for (int i = 3; i < idd.size(); i += 4) {   // ТРЕТИЙ ИНДЕКС = НОМЕР
            if (idd.get(i).isBlank()) {
                continue;
            } else if(idd.get(i).chars().equals(idd.get(i).chars()) || idd.get(i).chars().count() == 1){
                return;
            } else {
                p.parse(idd.get(i));
            }
